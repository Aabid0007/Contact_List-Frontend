...
import EditContact from '../Crud/EditContact/EditContact';
...
const ContactContainer = () => {
    ...
    return (
        <div className='page_content_section_table'>
                ...
                <tbody>
                    {Array.isArray(data) && data.map((contact, index) => (
                        <tr key={contact._id}>
                            ...
                            <td><div className='profile'><img src={`https://contact-list-backend-0zle.onrender.com/${contact.image}`} alt="" />{`${contact.firstName} ${contact.lastName}`}</div></td>
                            <td>{contact.email}</td>
                            <td>{contact.phone}</td>
                            <td>
            ...
            {editModalOpen && <EditContact editModalClose={() => setEditModalOpen(false)} contactId={editModalOpen} />}
            {deleteModalOpen && <DeleteContact deleteModalClose={() => setdeleteModalOpen(false)} contactId={deleteModalOpen} />}
        </div>
    )
}
